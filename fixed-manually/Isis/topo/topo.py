    ...
    def __init__(self, sw_path, thrift_port, pcap_dump, n, **opts):
        ...
        for h in xrange(e):
            switch = self.addSwitch('e%d' % (h + 1),
                                    sw_path = sw_path,
                                    json_path = "/home/vagrant/ic/MPolKA-INT/int-mpolka/m-polka-int-edge.json",
        ...
        for h in xrange(m):
            switch = self.addSwitch('s%d' % (h + 1),
                                    sw_path = sw_path,
                                    json_path = "/home/vagrant/ic/MPolKA-INT/int-mpolka/m-polka-int-core.json",
                                    thrift_port = thrift_port,
                                    pcap_dump = pcap_dump,
                                    log_console = True)
            self.switch_list.append(switch)
...
def main():
    ...
    print('before flowtable')

    os.system("../flow_table/f.sh")

    print('after flowtable')
